...
import { faUserGraduate, faChalkboardTeacher, faHome, faArrowRight } from '@fortawesome/free-solid-svg-icons';
...
const programs = [
  {
    ...
    description: 'Comprehensive self-paced lessons covering all levels from primary to university entrance exams',
...
const ProgramsSection: React.FC = () => {
  ...
  return (
    <section id="programs" className="py-20 bg-gradient-to-b from-white to-light-gray/30" ref={sectionRef}>
        ...
        >
          {programs.map((program, index) => (
            <motion.div
              ...
              custom={index}
              whileHover="hover"
              className={`relative p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all border-t-4 ${program.border} flex flex-col h-full overflow-hidden group`}
            >
              {/* Background accent */}
              <div className={`absolute top-0 right-0 w-32 h-32 -mr-16 -mt-16 rounded-full ${program.accent} opacity-50 group-hover:opacity-70 transition-opacity duration-500`}></div>
