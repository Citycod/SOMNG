...
const Navbar: React.FC = React.memo(() => {
  ...
  const renderMenuItem = (item: MenuItem, index: number, isMobile: boolean = false) => (
    <li key={index} className="group">
      ...
      >
        ...
        {!isMobile && (
          <div 
            className={`w-2 h-2 ml-2 rounded-full bg-gold ${activeItem === item.to ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
          />
        )}
      </ScrollLink>
    </li>
  ...
  return (
    <nav
      className="fixed top-0 w-full bg-white/95 backdrop-blur-md shadow-md transition-all duration-300 ease-in-out z-50 py-3"
      role="navigation"
